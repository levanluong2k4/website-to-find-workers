const fs = require('fs');
const file = 'resources/views/worker/my-bookings.blade.php';

console.log('Reading:', file);
let content = fs.readFileSync(file, 'utf8');

const NEW_MODAL = `<div class="modal fade dispatch-modal dispatch-modal--pricing-v2" id="modalCosts" tabindex="-1" aria-hidden="true">
          ...
          <footer class="dispatch-pricing-v2-footer">
             <div class="container-fluid">
                <div class="d-flex justify-content-end align-items-center gap-3">
                   ...
                   <button type="submit" class="dispatch-pricing-v2-btn dispatch-pricing-v2-btn--primary">
...
.dispatch-modal__title { color: #1c1b1b; font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: 1.4rem; }
...
.dispatch-pricing-v2-context-title { font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: 1.6rem; color: #1c1b1b; margin: 0 0 16px 0; }
...
.dispatch-pricing-v2-section-title { font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: 1.15rem; color: #1c1b1b; margin: 0; }
...
.dispatch-pricing-v2-part-title { font-family: 'DM Sans', sans-serif; font-weight: 700; font-size: 1rem; color: #1c1b1b; margin: 0 0 4px 0; }
...
.dispatch-pricing-v2-fee-title { font-family: 'DM Sans', sans-serif; font-weight: 700; font-size: 1rem; color: #1c1b1b; margin: 0 0 16px 0; }
...
.dispatch-pricing-v2-fee-value { font-family: 'DM Sans', sans-serif; font-weight: 800; color: #334155; font-size: 1.1rem; }
...
.dispatch-pricing-v2-summary-title { font-family: 'DM Sans', sans-serif; font-weight: 800; font-size: 1.25rem; margin: 0; }
...
.dispatch-pricing-v2-total-value { font-family: 'DM Sans', sans-serif; font-weight: 900; font-size: 2.4rem; color: #0056d2; letter-spacing: -0.04em; line-height: 1.1; margin-top: 8px; }
...
const NEW_JS = `const buildCostItemRowMarkup = (type, item = {}) => {
  ...
  if (isPart) {
    ...
    return \`
      <tr class="dispatch-line-item" data-line-type="\${type}">
        ...
        <td class="px-3 py-2 text-end" style="width: 180px;">
        ...
        <td class="px-2 py-2 text-center" style="width: 50px;">
           <button type="button" class="dispatch-pricing-v2-remove-btn dispatch-line-item__remove mx-auto">
              <span class="material-symbols-outlined">close</span>
           </button>
        </td>
      </tr>
...
const idx1 = content.indexOf('const buildCostItemRowMarkup = (type, item = {}) => {');
const idx2 = content.indexOf('};\r\n\r\nconst populateCostItemRows =');
if(idx1 > -1 && idx2 > -1) {
  content = content.substring(0, idx1) + NEW_JS + content.substring(idx2 + 2);
  console.log('JS replaced.');
} else {
  // try fallback with \n
  const f_idx2 = content.indexOf('};\n\nconst populateCostItemRows =');
  if(idx1 > -1 && f_idx2 > -1){
      content = content.substring(0, idx1) + NEW_JS + content.substring(f_idx2 + 2);
      console.log('JS replaced (fallback \\n).');
  } else {
      console.log('JS BOUNDS NOT FOUND');
  }
}

console.log('Replacing Modal...');
const m1 = content.indexOf('<div class="modal fade dispatch-modal dispatch-modal--pricing" id="modalCosts"');
const m2 = content.indexOf('</div>\r\n</div>\r\n\r\n<div class="modal fade dispatch-modal" id="modalViewDetails"');
if(m1 > -1 && m2 > -1) {
  content = content.substring(0, m1) + NEW_MODAL + '\r\n\r\n' + content.substring(m2);
  console.log('Modal replaced.');
} else {
  const fm2 = content.indexOf('</div>\n</div>\n\n<div class="modal fade dispatch-modal" id="modalViewDetails"');
  if(m1 > -1 && fm2 > -1) {
     content = content.substring(0, m1) + NEW_MODAL + '\n\n' + content.substring(fm2);
     console.log('Modal replaced (fallback \\n).');
  } else {
     console.log('MODAL BOUNDS NOT FOUND');
  }
}
