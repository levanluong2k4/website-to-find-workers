...
class SyncAiKnowledge extends Command
{
    protected $signature = 'app:sync-ai-knowledge
        {--source=all : all|booking_case|service_catalog}
    ...
    public function handle(AiKnowledgeSyncService $syncService): int
    {
        ...
        if (!in_array($source, ['all', 'booking_case', 'service_catalog'], true)) {
            $this->error('Gia tri --source khong hop le. Su dung: all, booking_case, service_catalog.');
            return self::FAILURE;
        }
