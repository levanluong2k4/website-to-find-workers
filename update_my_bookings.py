...
filepath = r"d:\laragon\www\DATN\website-to-find-workers\resources\views\worker\my-bookings.blade.php"

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# Normalize content endings to \n temporarily
content = content.replace('\r\n', '\n')

# 1. New HTML for modalCosts
new_html = r"""<div class="modal fade dispatch-modal dispatch-modal--pricing-v2" id="modalCosts" tabindex="-1" aria-hidden="true">
...
.dispatch-modal__header-divider {
  width: 1px;
  height: 24px;
  background: rgba(195, 198, 214, 0.4);
}
...
.dispatch-pricing-v2-stepper-btn {
  ...
  justify-content: center;
  cursor: pointer;
  box-shadow: 0 1px 2px rgba(0,0,0,0.05); border: 1px solid rgba(195,198,214,0.3);
}
...
.dispatch-pricing-v2-summary-status {
  background: rgba(15, 159, 124, 0.15);
  color: #0f9f7c;
  font-size: 0.75rem;
  font-weight: 700;
  padding: 4px 12px;
...
.dispatch-pricing-v2-footer {
  ...
  border-top: 1px solid rgba(195, 198, 214, 0.4);
  padding: 16px 24px;
  z-index: 100;
...
.dispatch-pricing-v2-btn--primary {
  ...
  color: #ffffff;
  box-shadow: 0 4px 14px rgba(13, 124, 193, 0.3);
}
.dispatch-pricing-v2-btn--primary:hover {
  transform: translateY(-1px);
  box-shadow: 0 6px 20px rgba(13, 124, 193, 0.4);
}
...
.dispatch-line-item {
  /* To help JS identify rows */
}
/* END PRICING V2 STYLES */
"""

# 3. New JS Template for buildCostItemRowMarkup
new_js_func = r"""
          ...
          <div class="d-flex gap-3 align-items-center mb-3 mb-md-0" style="flex: 1;">
            ...
            <div class="flex-grow-1">
              <h4 class="dispatch-pricing-v2-part-title m-0">
                <input type="text" class="dispatch-pricing-v2-input-dark js-line-description m-0" style="background:transparent; border:none; padding:0; padding-bottom: 4px; box-shadow:none;" value="${description}" placeholder="Bo mạch..." ${isCatalogItem ? 'readonly' : ''}>
              </h4>
...
# Replace Modal
idx_start_modal = content.find('<div class="modal fade dispatch-modal dispatch-modal--pricing" id="modalCosts"')
end_marker = '</div>\n</div>\n\n<div class="modal fade dispatch-modal" id="modalViewDetails"'
idx_end_modal = content.find(end_marker)

if idx_start_modal != -1 and idx_end_modal != -1:
    content = content[:idx_start_modal] + new_html + '\n\n' + content[idx_end_modal:]
else:
    print("Could not find modal bounds.")
    sys.exit(1)


# Replace JS buildCostItemRowMarkup
idx_start_js = content.find('const buildCostItemRowMarkup = (type, item = {}) => {')
idx_end_js = content.find('};\n\nconst populateCostItemRows =')
if idx_start_js != -1 and idx_end_js != -1:
    content = content[:idx_start_js] + new_js_func.strip() + content[idx_end_js+2:]
else:
    print("Could not find js bounds.")
    sys.exit(2)
...
# Important: write back with the system's original ending style (optional, opening with 'w' typically does this on win)
