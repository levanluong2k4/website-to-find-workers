...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .otp-title{font-family:'DM Sans',sans-serif;font-weight:700;font-size:1.5rem;color:#0f172a;margin-bottom:.5rem;}
    ...
    .otp-input{width:58px;height:70px;border:2px solid #e2e8f0;border-radius:.875rem;font-family:'DM Sans',sans-serif;font-size:1.75rem;font-weight:800;text-align:center;color:#0f172a;background:#f8fafc;transition:all .2s;outline:none;}
    ...
    /* Submit button */
    .btn-submit{width:100%;background:linear-gradient(135deg,#0EA5E9,#0284c7);color:#fff;border:none;border-radius:1rem;padding:1rem;font-family:'DM Sans',sans-serif;font-weight:700;font-size:1rem;cursor:pointer;transition:all .2s;display:flex;align-items:center;justify-content:center;gap:.5rem;margin-bottom:1.25rem;}
...
document.getElementById('otpForm').addEventListener('submit', async e => {
  e.preventDefault();
  if (!role) return showToast('Thiếu vai trò đăng nhập, vui lòng quay lại đăng nhập lại.', 'error');
  let otpCode = '';
  ...
  try {
    const res = await callApi('/verify-otp', 'POST', { email, code: otpCode, role });
    if (res.ok) {
      const { access_token, user } = res.data;
      saveUserSession(access_token, user);
      if (res.data.requires_phone_verification) {
        showToast('Email da xac thuc. Vui long xac minh them so dien thoai.');
        setTimeout(() => {
          window.location.href = res.data.phone_verification_url || (baseUrl + '/verify-phone');
        }, 700);
        return;
      }
      try {
...
window.resendOtp = async function() {
  ...
  try {
    const res = await callApi('/resend-otp', 'POST', { email, role });
    if (res.ok) {
